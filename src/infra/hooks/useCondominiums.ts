...
import { ICondominiumsResponse, condominiumService } from "../services/condominium";

interface ISyndicResponseHook {
    condominiums: ICondominium[] | [];
    condominiumsLoading: boolean;
    condominiumsRefresh: VoidFunction;
    condominiumsTotal: number;
}
...
export const useCondominiums = ({
    ...
    const [data, setData] = useState<ICondominium[]>([]);
    ...
    const fetchData = () => {
        ...
        condominiumService
            .getAll(query)
            .then((res: AxiosResponse<ICondominiumsResponse>) => {
    ...
    return {
        condominiums: data,
        condominiumsLoading: loading,
        condominiumsRefresh: fetchData,
        condominiumsTotal: total,
    };
};
