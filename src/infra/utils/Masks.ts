const Masks = {
    ...
    clearCNPJOrCPF(value: string): string {
        if (value.length <= 14) {
            return this.clearCPF(value);
        }

        return this.clearCNPJ(value);
    },
    ...
    clearRG(value: string): string {
        return value.replace(/\D+/g, "");
    },

    clearCNPJ(value: string): string {
        return value.replace(/\D+/g, "");
    },
    ...
    clearPhone(value: string): string {
        return value.replace(/\D+/g, "");
    },

    clearCEP(value: string): string {
        return value.replace(/\D+/g, "");
    },
    ...
    clearMoney(value: string): number {
        const cleanValue = value.replace(/\D/g, ""); // Remove tudo que não for número
        const valueConverted = parseFloat(cleanValue) / 100; // Converte para decimal correto

        return valueConverted;
    },

    justNumbers(value: string | number): number {
        return (
            +parseFloat(
                value
                    ?.toString()
    ...
    justNumbersWithPoints(value: string | number): string {
        const cleanedValue = value.toString().replace(/[^0-9.,]/g, "");
        const normalizedValue = cleanedValue.replace(/\./g, "");
        return normalizedValue.replace(",", ".");
    },

    limitText(value: string, limit: number = 50): string {
        return value.length < limit ? value : value.substring(0, limit) + "...";
    },

    noAccents(str: string): string {
